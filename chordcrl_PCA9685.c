...
void presschord(int chord_channel) {
    ...
    int pulse_width = map(PRESS_ANGLE, 0, 180, SERVO_MIN, SERVO_MAX); // サーボの角度を計算してもらう
    setPWM(chord_channel, 0, pulse_width); // PWMでサーボを動かす
    printf("Pressed chord: %s (Channel %d) at angle %d\n", chordlist[chord_channel], chord_channel, PRESS_ANGLE);
