...
// サーボの角度設定
#define OPEN_ANGLE 0   // 開放弦の角度（仮）

// channel6は、右か左しかないからね
#define LEFT_ANGLE 20  
#define RIGHT_ANGLE 170

// これ何に使ってるかわかんねぇな
#define SUBSERVO01 25 // サブサーボの角度
#define SUBSERVO02 45 // サブサーボの角度
...
// chords_mapping の各サブ配列には、chordごとにサーボのチャンネルと角度情報を格納している
// 例: Cメジャーコードの場合、chords_mapping[0]は{{4, 170}, {6, LEFT_ANGLE}}となり、4番チャンネルを170度、6番チャンネルをLEFT_ANGLEに設定する
// 2つ目以降の{0, -1}は、そのコードで使わないサーボのダミー値として用いる
// サーボチャンネルは通常 4, 5, 6, 7 に対応
// このデータ構造を参照して presschord などの関数でサーボの操作を行う
int chord_channels_count[] = {2, 2, 2, 2, 2, 2, 2, 2, 2}; // Number of servos for each chord

void i2c_write_byte(int reg, int value) { // I2Cに1バイト書き込む関数
...
// サーボをすべてオフにする（開放弦）
void openPWM(int channel, int angle) {
    
    // int pulse_width = map(angle, 0, 180, SERVO_MIN, SERVO_MAX);
    // setPWM(channel, (SERVO_MIN + SERVO_MAX)/2);
    // setPWM(channel, OPEN_ANGLE);
...
void allopen() {
    ...
    for (int i = 4; i < 8; i++) { // ループ回数を10に修正
        pulse_width = map(channel_angle[i-4], 0, 180, SERVO_MIN, SERVO_MAX); // 各サーボの角度に設定
        switch (i) {
            case 4:
                setPWM(i, pulse_width);
                break;
            case 5:
                setPWM(i, pulse_width);
                break;
...
void presschord(int chord_channel) {
    ...
    if (last_chord != -1 && last_chord != chord_channel) { // 前回のサーボをオフにする
        ...
        for (int i = 0; i < last_num_channels; i++) {
            ...
            int angle = map(channel_angle[channel - 4],0, 180, SERVO_MIN, SERVO_MAX);;
            openPWM(channel,angle);
    ...
    for (int i = 0; i < num_channels; i++) { // 押さえたいコードを抑える
        ...
        setPWM(channel, angle);
        printf("Set channel %d to angle %d\n", channel, chords_mapping[chord_channel][i].angle);
...
int setup(void){ // setup でPCA9685の初期化を行う
